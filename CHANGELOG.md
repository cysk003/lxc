...
- 统一无交互入口为 `export noninteractive=true`，保留旧环境变量兼容
- 补齐安装、卸载、批量新增和 swap 脚本的无交互参数校验
- 修复 LXD 安装状态文件命名、自定义存储路径记录和卸载时 IPv6 持久化服务清理
- 优化容器/虚拟机创建参数校验，减少虚拟机获取 IP 时的重复 LXC 查询
- 修复 Alpine/OpenWrt SSH 初始化中的 POSIX `sh` 兼容和 `chpasswd` 调用
- 修复 IPv6 nftables 持久化恢复对 `grep -P` 的依赖
- 优化容器/虚拟机代理端口设备重复配置时的替换和清理逻辑
- 优化容器内包屏蔽、监控脚本和 Docker 临时脚本的非交互与幂等行为
- 修复监控 screen 会话名拼写并让监控脚本下载失败时明确退出
- 补强核心远程脚本下载失败处理，避免继续执行空文件或旧残留文件
- 避免重复安装时重复添加 opsmaru remote 和 iptables MASQUERADE 规则
- 收紧 IPv6 网络映射脚本的参数引用并移除包屏蔽脚本写入中的 `sh -c` 拼接
- 修复启用 IPv6 时覆盖宿主机或容器既有 crontab 的问题
- 清理颜色输出函数和虚拟机换源流程中的静态检查错误
...
- 自5.x版本后废弃了--storage-pool-dir参数，替换使用官方最新的存储池设置方式

2025.11.05

- 添加从路由器获取真实IPV6子网掩码的功能，增强SLAAC下的信息获取
- 增加Perl兼容模式自动替换的功能

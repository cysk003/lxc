...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/lxd-batch-transaction.XXXXXX")
...
run_add_case() (
    ...
    export NEW_NUMS=2
    export CPU_NUMS=1
    export MEMORY_NUMS=128
    export DISK_NUMS=1
    export INPUT_NUMS=100
    export OUTPUT_NUMS=100
    export ENABLE_IPV6=N
    export SYSTEM_IMAGE=debian12
...
run_main_configuration_failure() (
    ...
    lxc() {
        ...
        init)
            ...
            ;;
        delete)
            rm -f -- "$case_dir/state/${3}.exists"
            return 0
            ;;
        config)
            if [[ "$*" == *'device override base root size=1GB'* ]]; then
                return 66
            fi
...
printf 'LXD batch transaction tests passed\n'
